...
public class SearchRequstByBFS {
    ...
    public void initSavePath(){
        if(report_save_path == null){
            this.result_file = String.format("%s_result_%s.txt",model_name, CommonUtil.getCurrentDate());
            this.all_chain_file = String.format("%s_log_%s.txt",model_name, CommonUtil.getCurrentDate());
            this.err_log_file = String.format("%s_error_%s.txt",model_name, CommonUtil.getCurrentDate());
        }else{
            this.result_file = String.format("%s/%s_result_%s.txt",report_save_path,model_name, CommonUtil.getCurrentDate());
            this.all_chain_file = String.format("%s/%s_log_%s.txt",report_save_path,model_name, CommonUtil.getCurrentDate());
            this.err_log_file = String.format("%s_error_%s.txt",report_save_path,model_name, CommonUtil.getCurrentDate());
        }
    }
            ...
            try {
                ...
                if (!visited.contains(filed_object)) {
                    ...
                    if (log_chain != null && log_chain != "") {
                        current_depth++;
                        new_log_chain = String.format("%s \n%s ---> %s = {%s}", log_chain, CommonUtil.getBlank(current_depth), filed_name, filed_object.getClass().getName());
                    } else {
                    ...
                    if (MatchUtil.matchObject(filed_name, filed_object, keys)) {
                        CommonUtil.write2log(result_file, new_log_chain + "\n\n\n");
                    }
                    if (is_debug) {
                        CommonUtil.write2log(all_chain_file, new_log_chain + "\n\n\n");
                    }
                    ...
                    for (; clazz != Object.class; clazz = clazz.getSuperclass()) {//向上循环 遍历父类
                        ...
                        for (Field field : fields) {
                            ...
                            if (subObj == null) {
                                ...
                                continue;
                            } else if (CheckUtil.isList(field)) {
                                //对List,ArrayList类型的属性遍历
                                try {
                                    List list = (List) field.get(filed_object);
                                    if (list != null && list.size() > 0) {
                                        current_depth++;
                                        String tmp_log_chain = String.format("%s \n%s ---> %s = {%s}", new_log_chain, CommonUtil.getBlank(current_depth), proName, proType);
                                        int len = list.size();
                                        for (int i = 0; i < len; i++) {
                                            if (list.get(i) == null) {
                                                continue;
